...
pub struct ExecuteTransaction<'info> {
    ...
    /// The bridging transaction account to be executed
    #[account(mut, seeds = [BRIDGING_TRANSACTION_SEED, validator_set.last_batch_id.to_le_bytes().as_ref()], bump = bridging_transaction.bump, close = payer)]
    ...
    #[account(address = bridging_transaction.receiver @ CustomError::InvalidReceiver)]
...
impl<'info> ExecuteTransaction<'info> {
    ...
    pub fn process_instruction(ctx: Context<Self>, amount: u64) -> Result<()> {
        ...
        require!(
            bridging_transaction.signers.len() as u8 >= ctx.accounts.validator_set.threshold,
        ...
        // Create the vault's associated token account if it doesn't exist
